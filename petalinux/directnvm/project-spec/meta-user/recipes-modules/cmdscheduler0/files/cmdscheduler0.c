...
MODULE_DESCRIPTION("cmdscheduler0 - CmdScheduler0 registers");

#define DRIVER_NAME "cmdscheduler0"
...
{
	pr_info("[cmdscheduler0] vma->vm_start: %px, vma->vm_end: %px, vma->vm_pgoff: %px",
	...
	vma->vm_ops = &vm_ops;
	vma_open(vma);
	pr_info("[cmdscheduler0] registers are mapped\n");
	return 0;
}
