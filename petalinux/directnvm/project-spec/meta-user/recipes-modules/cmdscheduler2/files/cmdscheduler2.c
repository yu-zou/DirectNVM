...
MODULE_DESCRIPTION("cmdscheduler2 - CmdScheduler2 controller registers");

#define DRIVER_NAME "cmdscheduler2"
...
{
	pr_info("[cmdscheduler2] vma->vm_start: %px, vma->vm_end: %px, vma->vm_pgoff: %px",
	...
	vma->vm_ops = &vm_ops;
	vma_open(vma);
	pr_info("[cmdscheduler2] registers are mapped\n");
	return 0;
}
