...
MODULE_DESCRIPTION("cmdscheduler1 - CmdScheduler1 controller registers");

#define DRIVER_NAME "cmdscheduler1"
...
{
	pr_info("[cmdscheduler1] vma->vm_start: %px, vma->vm_end: %px, vma->vm_pgoff: %px",
	...
	vma->vm_ops = &vm_ops;
	vma_open(vma);
	pr_info("[cmdscheduler1] registers are mapped\n");
	return 0;
}
