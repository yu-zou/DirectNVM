...
MODULE_DESCRIPTION("cmdscheduler3 - CmdScheduler3 controller registers");

#define DRIVER_NAME "cmdscheduler3"
...
{
	pr_info("[cmdscheduler3] vma->vm_start: %px, vma->vm_end: %px, vma->vm_pgoff: %px",
	...
	vma->vm_ops = &vm_ops;
	vma_open(vma);
	pr_info("[cmdscheduler3] registers are mapped\n");
	return 0;
}
