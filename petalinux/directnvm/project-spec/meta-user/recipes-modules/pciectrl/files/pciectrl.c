...
MODULE_DESCRIPTION("pciectrl - PCIe controller registers");

#define DRIVER_NAME "pciectrl"
...
{
	pr_info("[pciectrl] Device is opened\n");
}
...
{
	pr_info("[pciectrl] vma->vm_start: %px, vma->vm_end: %px, vma->vm_pgoff: %px\n",
	...
	vma->vm_ops = &vm_ops;
	vma_open(vma);
	pr_info("[pciectrl] registers are mapped\n");
	return 0;
}
