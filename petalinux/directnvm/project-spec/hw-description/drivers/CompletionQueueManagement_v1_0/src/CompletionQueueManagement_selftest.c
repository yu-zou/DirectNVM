...
/***************************** Include Files *******************************/
#include "CompletionQueueManagement.h"
...
XStatus COMPLETIONQUEUEMANAGEMENT_Mem_SelfTest(void * baseaddr_p)
{
	...
	{
	  COMPLETIONQUEUEMANAGEMENT_mWriteMemory(baseaddr+4*Index, (0xDEADBEEF % Index));
	}

	for ( Index = 0; Index < 16; Index++ )
	{
	  Mem32Value = COMPLETIONQUEUEMANAGEMENT_mReadMemory(baseaddr+4*Index);
