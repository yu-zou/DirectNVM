# Runtime Tcl commands to interact with - CompletionQueueManagement_v2_0

# Sourcing design address info tcl
set bd_path [get_property DIRECTORY [current_project]]/[current_project].srcs/[current_fileset]/bd
source ${bd_path}/CompletionQueueManagement_v2_0_include.tcl
...
# Test: S00_AXI_FULL
# Create a burst write transaction at s00_axi_full_addr address
create_hw_axi_txn w_s00_axi_full_addr [get_hw_axis $jtag_axi_master] -type write -address $s00_axi_full_addr -len 4 -data $wdata_2 -burst INCR
# Create a burst read transaction at s00_axi_full_addr address
create_hw_axi_txn r_s00_axi_full_addr [get_hw_axis $jtag_axi_master] -type read -address $s00_axi_full_addr -len 4 -burst INCR
# Initiate transactions
run_hw_axi r_s00_axi_full_addr
run_hw_axi w_s00_axi_full_addr
run_hw_axi r_s00_axi_full_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_s00_axi_full_addr]]
# Compare read data
if { $rdata_tmp == $wdata_2 } {
	puts "Data comparison test pass for - S00_AXI_FULL"
} else {
	puts "Data comparison test fail for - S00_AXI_FULL, expected-$wdata_2 actual-$rdata_tmp"
	inc ec
}
...
# Test: M00_AXI_LITE
set wdata_m00_axi_lite [get_init_data 0]
create_hw_axi_txn w_m00_axi_lite_addr [get_hw_axis $jtag_axi_master] -type write -address $axi_gpio_out_addr -data ${wdata_m00_axi_lite}
create_hw_axi_txn r_m00_axi_lite_addr [get_hw_axis $jtag_axi_master] -type read -address $axi_gpio_in_addr 
# Initiate transactions
run_hw_axi r_m00_axi_lite_addr
run_hw_axi w_m00_axi_lite_addr
run_hw_axi r_m00_axi_lite_addr
set rdata_tmp [get_property DATA [get_hw_axi_txn r_m00_axi_lite_addr]]
set DE [ get_done_and_error_bit $rdata_tmp 1 0 ]
# Compare read data
if { $DE == 01 } {
	puts "Data comparison test pass for - M00_AXI_LITE"
} else {
	puts "Data comparison test fail for - M00_AXI_LITE, rdata-$rdata_tmp expected-01 actual-$DE"
	inc ec
}
